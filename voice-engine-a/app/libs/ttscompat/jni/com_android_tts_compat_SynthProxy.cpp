...
#define ALOGE_IF(_, ...) while(false);
#define LOGE_EX(_, ...) while(false);
#define assert(_, ...) while(false);
...
// NB(xenosoz, 2018): We use std::lock_guard for replacement of Mutex::Autolock
//   See also std::scoped_lock if we can assume we're in C++17 world.
...
jint JNI_OnLoad(JavaVM* vm, void* reserved)
{
    ...
    if (jniRegisterNativeMethods(
            ...
            env, "com/enuma/voice_engine_a/core/SynthProxy", gMethods, NELEM(gMethods)) < 0) {
        return -1;
    }
