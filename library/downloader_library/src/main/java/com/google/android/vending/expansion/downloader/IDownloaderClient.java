 ...
 * downloader. It is used to pass status from the services to the client.
 */
public interface IDownloaderClient {
     ...
     * cellular permission will restart the services. Wi-Fi disabled means that
     ...
     * services. Wi-Fi disabled means that the Wi-Fi manager is returning that
     * Wi-Fi is not enabled, while in the other case Wi-Fi is enabled but not
     * available.
     * <p>
     * The services does not return these values. We recommend that app
    ...
    /**
     * Called internally by the stub when the services is bound to the client.
     * <p>
     * Critical implementation detail. In onServiceConnected we create the
     * remote services and marshaler. This is how we pass the client information
     * back to the services so the client can be properly notified of changes. We
     * must do this every time we reconnect to the services.
     * <p>
     ...
     * 
     * @param m the services Messenger. This Messenger is used to call the
     *            services API from the client.
     */
    void onServiceConnected(Messenger m);

    /**
     * Called when the download state changes. Depending on the state, there may
     * be user requests. The services is free to change the download state in the
