...
Worksheet& LevelData::randomSheetFor(const string& LanguageTag, size_t LevelID, size_t *SheetID) {
    auto K = LevelKey(LanguageTag, LevelID);
    auto& L = Levels[K];

    *SheetID = random(L.begin_index(), L.end_index()-1);
    ...
    for (auto it : Levels ) {
        ...
        for (auto sheet : sheets) {
            for (int i = sheet.beginProblemID(); i<sheet.endProblemID(); i++) {
                ...
                SoundEffect s2;
                string v = "WordTrace/BonusVideos/" + p.VideoFileName;
